...
class ConfigMapReconciler implements ResourceReconciler<V1ConfigMap> { // <2>
    ...
    private static final Logger LOG = LoggerFactory.getLogger(ConfigMapReconciler)
    ...
    @Override
    @NonNull
    Result reconcile(@NonNull Request request, @NonNull OperatorResourceLister<V1ConfigMap> lister) { // <3>
        ...
        if (configMapOpt.isPresent()) {
            ...
            V1ObjectMeta metadata = configMap.metadata

            Map<String, String> annotations = metadata.annotations
            if (annotations == null) {
