...
You can also enable fail-fast property in case you want an exception to be thrown if at least one of the labels,
specified in pod-labels section is not found in the namespace. This can be useful if you want to prevent loading
all config maps available in namespace in case of mistyping or misconfiguration:
